...
const DashBoard = () => {
  ...
  return (
    <>
      <TopBar title='Painel' />
      <Container sx={{ mt: 12 }}>
        <Typography component='h1' mb={10} sx={{ fontSize: '32px', fontWeight: '700', textAlignLast: 'left', color: 'neutral.main' }}>Olá, Fulano!</Typography>
        <Box sx={{
          display: 'flex',
          ...
          <Box>
            <Typography component='h2' mb={3} sx={{ fontSize: '18px', fontWeight: '700', textAlignLast: 'left', color: 'neutral.main' }}>Lista de projetos</Typography>
            <Typography mb={5} sx={{ color: '#565656', fontSize: '16px', textAlignLast: 'left', }}>Confira as campanhas que foram realizadas anteriormente.</Typography>
          </Box>
          <TextField
                  ...
                  <Paper sx={{ height: '80px', p: '18px 0 0 28px' }} key={item.id} elevation={1}>
                    <Box sx={{ display: 'grid', alignItems: 'center', gridTemplateColumns: '7fr 2fr 2fr 2fr 1fr' }}>
                      ...
                      <Box>
                        ...
                        <Typography variant='body' sx={{ color: '#565656', fontSize: '16px', fontWeight: '400' }}>{item.campaignChannel === 'EMKT' ? 'Email Marketing' : item.campaignChannel}</Typography>
